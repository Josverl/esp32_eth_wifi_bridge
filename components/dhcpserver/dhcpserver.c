...
/* Vendored extension: static MAC->IP reservations (see dhcps_reservations.h) */
...
/* --- Vendored extension: static reservation resolver (single instance) --- */
static dhcps_resolve_mac_fn s_resv_lookup;
static dhcps_reserved_ip_fn s_resv_ip_taken;
static void                *s_resv_arg;

void dhcps_set_reservation_resolver(dhcps_resolve_mac_fn lookup,
                                    dhcps_reserved_ip_fn ip_taken,
                                    void *arg)
{
    s_resv_lookup   = lookup;
    s_resv_ip_taken = ip_taken;
    s_resv_arg      = arg;
}
...
static s16_t parse_msg(dhcps_t *dhcps, struct dhcps_msg *m, u16_t len)
{
    ...
    if (memcmp((char *)m->options, &magic_cookie, sizeof(magic_cookie)) == 0) {
        ...
        /* --- Vendored extension: honour static MAC->IP reservations ---
         * If this client MAC has a reservation, hand out the fixed address
         * (which may legitimately sit outside the dynamic pool) instead of
         * allocating from the pool. */
        if (s_resv_lookup != NULL) {
            u32_t resv_ip = 0;
            if (s_resv_lookup(s_resv_arg, m->chaddr, &resv_ip)) {
                struct dhcps_pool *rpool = NULL;
                list_node *rnode = NULL;

                /* Reuse an existing pool entry for this MAC if present. */
                for (pback_node = dhcps->plist; pback_node != NULL;
                     pback_node = pback_node->pnext) {
                    struct dhcps_pool *pp = pback_node->pnode;
                    if (memcmp(pp->mac, m->chaddr, sizeof(pp->mac)) == 0) {
                        rpool = pp;
                        rnode = pback_node;
                        break;
                    }
                ...
                if (rpool == NULL) {
                    rpool = (struct dhcps_pool *)mem_calloc(1, sizeof(struct dhcps_pool));
                    rnode = (list_node *)mem_calloc(1, sizeof(list_node));
                    if (rpool != NULL && rnode != NULL) {
                        memcpy(rpool->mac, m->chaddr, sizeof(rpool->mac));
                        rnode->pnode = rpool;
                        rnode->pnext = NULL;
                        node_insert_to_list(&dhcps->plist, rnode);
                    } else {
                        /* Allocation failed: fall back to dynamic handling. */
                        if (rpool != NULL) { free(rpool); }
                ...
                if (rpool != NULL) {
                    rpool->ip.addr = resv_ip;
                    rpool->lease_timer = lease_timer;
                    dhcps->client_address.addr = resv_ip;
                    if (memcmp(&resv_ip, m->ciaddr, sizeof(resv_ip)) == 0) {
                        dhcps->renew = true;
                    }

                    s16_t ret = parse_options(dhcps, &m->options[4], len);
                    if (ret == DHCPS_STATE_RELEASE || ret == DHCPS_STATE_NAK) {
                        node_remove_from_list(&dhcps->plist, rnode);
                        free(rnode);
                        free(rpool);
        ...
        if (dhcps->client_address.addr > dhcps->dhcps_poll.end_ip.addr) {
            ...
            /* Vendored extension: never hand out an address that is statically
             * reserved for a different client; skip forward until a free,
             * unreserved address is found (or the pool is exhausted). */
            if (s_resv_ip_taken != NULL) {
                while (s_resv_ip_taken(s_resv_arg, dhcps->client_address.addr, m->chaddr)) {
                    addr_tmp.addr = htonl(dhcps->client_address.addr);
                    addr_tmp.addr++;
                    dhcps->client_address.addr = htonl(addr_tmp.addr);
                    if (dhcps->client_address.addr > dhcps->dhcps_poll.end_ip.addr) {
                        break;
                    }
                }
